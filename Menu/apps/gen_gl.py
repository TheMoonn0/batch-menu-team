...
# CSV จาก ZIP, TLF (.xlsx) จาก folder Data_GL
# =========================
...
def process_combined_data_from_zip(zip_bytes: bytes, source_folder_name: str, tlf_folder_path: str = DEFAULT_TLF_FOLDER):
    ...
    # ✅ อ่าน TLF (.xlsx) จาก folder Data_GL
    if not os.path.isdir(tlf_folder_path):
        raise ValueError(f"ไม่พบ folder TLF: {tlf_folder_path}")

    tlf_files = sorted(
        [f for f in os.listdir(tlf_folder_path) if f.lower().endswith('.xlsx')],
    ...
    for f in tlf_files:
        try:
            full_path = os.path.join(tlf_folder_path, f)
            book = pd.ExcelFile(full_path)
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes), 'r') as z:
        ...
        log(f"TLF files: {[f for f, _ in tlf_books]}")
